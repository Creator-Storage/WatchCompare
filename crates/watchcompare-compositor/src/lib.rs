...
impl Canvas {
    pub fn new(width: u32, height: u32, clear: Color) -> Self {
        Self {
            image: RgbaImage::from_pixel(width, height, clear.pixel()),
        }
    }

    pub fn reference(clear: Color) -> Self {
        Self::new(REFERENCE_WIDTH, REFERENCE_HEIGHT, clear)
    }
    ...
    pub fn fill_rect(&mut self, rect: RectI32, color: Color) {
        ...
        let x1 = (rect.x.saturating_add(rect.width as i32))
            .max(0)
            .min(self.width() as i32) as u32;
        let y1 = (rect.y.saturating_add(rect.height as i32))
            .max(0)
    ...
    /// Deterministic scanline fill using pixel centers. This intentionally does
    /// not claim to match After Effects anti-aliasing yet; the image-diff loop
    /// will choose the final edge rasterization rule.
    ...
    /// Draws a feathered diagonal band in destination coordinates. `axis_degrees`
    /// is the direction of the band itself; `center_normal_px` is position along
    /// the perpendicular normal. This is the primitive used by the badge-shine
    /// calibration pass.
    pub fn diagonal_band(
        &mut self,
        clip: RectI32,
        axis_degrees: f32,
        ...
        for y in y0..y1 {
            for x in x0..x1 {
                ...
                if distance > half_width_px + feather {
                    continue;
                }
                let coverage = if distance <= half_width_px {
                    1.0
                } else {
                    1.0 - (distance - half_width_px) / feather
                };
    ...
    /// Draws glyphs at an explicit baseline using a caller-provided font. No
    /// platform text API is involved, so Windows and Android consume the same
    /// glyph rasterizer. Kerning/layout calibration remains a reference-diff job.
...
/// Returns the exact timeline/object state consumed by this compositor for a
/// source frame. Rendering code must not re-derive animation timing separately.
pub fn scene_for_frame(frame: u64) -> ReferenceSceneState {
...
mod tests {
    ...
    fn polygon_fill_stays_inside_measured_shape_bounds() {
        ...
        canvas.fill_polygon(
            &[(10.0, 1.0), (18.0, 5.0), (18.0, 15.0), (10.0, 19.0), (2.0, 15.0), (2.0, 5.0)],
            Color::WHITE,
        );
